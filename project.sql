...
and salary not in(4500,10,000,15,000);
...
use HR;
select department_name from departments group by (department_name) having sum(location_id)>2000; 

selecT state_province
from locations
where state_province is not  null; /*for not null*/
...
/*Q4*/
select department_id , sum(salary)
from employees where department_id='103' group by department_id;
...
/* add foreign key*/
use HR;
create table Persons(person_id int, firstname varchar(50), lastname varchar(50) , age int , primary key(person_id));

insert into Persons values(1,'lija','kasvala',18),(2,'princy' ,'patel',17);
...
alter table orders add constraint orders_ibfk_1 foreign key (person_id) references Persons(person_id);

/*check constraint*/
alter table persons add check (age>=16);

/*drop check constraint*/
alter table persons add constraint chk_personage check (age>=16 );
...
/* subQuery*/
select first_name , last_name ,salary from employees where salary>=(select salary from employees where last_name = 'Kochhar');
select first_name , last_name , salary from employees where salary>(select Avg(salary) from employees);


/*subquery with ALL,ANY, IN, or SOME*/
select first_name,salary from employees where salary  in(select salary from employees where salary>12000);
select first_name ,salary from employees where salary <> all (select first_name from employees);


select department_id , avg(salary) from employees group by department_id having avg (salary)>=all 
(select avg(Salary) from employees group by department_id);

select first_name , last_name , department_id from employees where department_id = any
(select department_id from departments where location_id = 1800);

select first_name from employees where row(department_id , manager_id) = 
(select department_id , manager_id from departments where location_id = 1800);
select*from departments;
...
from departments 
where not exists(select*from employees where department_id = department_id);
...
use HR;
/* join*/
select persons.person_id , persons.firstname , orders.order_no from persons inner join orders on persons.person_id = orders.person_id;
select persons.person_id , persons.firstname, orders.order_no from persons left join orders on persons.person_id = orders.person_id;
select persons.person_id , persons.firstname, orders.order_no from persons right outer join orders on persons.person_id = orders.person_id;
